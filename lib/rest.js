...
// const fixieRequest = process.env.MAKE_FIXIE ? request.defaults({'proxy': process.env.FIXIE_URL}) : request;
const fixieUrl = process.env.FIXIE_URL
const fixieEnabled = process.env.FIXIE_ENABLED
...
class BinanceRest {
    ...
    _makeRequest(query, callback, route, security, method, attempt = 0, useFixie = false) {
        assert(
        ...
        if (useFixie && fixieUrl && fixieEnabled) {
            options.proxy = fixieUrl;
        }

        const action = cb => {
            // const rod = this.fixie ? fixieRequest : request
    ...
    klines(query = {}, callback) {
        // this.fixie = useFixie && fixieUrl ? true : false
    ...
    // Private APIs
    newOrder(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
    ...
    testOrder(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
        this._setTimestamp(query);
        return this._makeRequest(
            query,
    ...
    openOrders(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
        if (_.isString(query)) {
            query = { symbol: query };
        }
    ...
    allOrders(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
        if (_.isString(query)) {
            query = { symbol: query };
        }
        this._setTimestamp(query);
        return this._makeRequest(query, callback, 'api/v3/allOrders', 'SIGNED', undefined, undefined, useFixie);
    }
    ...
    orderOco(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
        this._setTimestamp(query);
        return this._makeRequest(
            query,
    ...
    queryOco(query = {}, callback, useFixie) {
        // this.fixie = useFixie && fixieUrl ? true : false
        this._setTimestamp(query);
        return this._makeRequest(
            query,
