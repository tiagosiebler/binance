...
class Beautifier {
    ...
    beautifyValueWithKey(key, val) {
        if (typeof key === 'string' && this._floatKeysHashMap[key]) {
    ...
    beautifyObject(data) {
        ...
        _.mapObject(data, function(val, key) {
            if (Array.isArray(val)) {
              beautifedObject[key] = inst.beautifyArray(val, key);
            } else {
              beautifedObject[key] = inst.beautifyValueWithKey(key, val);
            }
        });
    ...
    beautifyArray(data, parentKey) {
        ...
        data.map(function (val, key) {
          if (Array.isArray(val)) {
            beautifedArray.push(inst.beautifyArray(val, parentKey || key));
          } else if (_.isObject(val)) {
            beautifedArray.push(inst.beautifyObject(val))
          } else {
            beautifedArray.push(inst.beautifyValueWithKey(parentKey || key, val));
          }
        });
