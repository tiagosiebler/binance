...
class BinanceWS {
    ...
    _setupWebSocket(eventHandler, path, isCombined) {
        ...
        ws.on('close', () => {
            for(let path in this._sockets){
                if(this._sockets[path] === ws){
    ...
    onUserData(binanceRest, eventHandler, interval = 60000) {
        return binanceRest.startUserDataStream()
            .then((response) => {
                let alive_keeper = setInterval(() => {
                    binanceRest.keepAliveUserDataStream(response);
                }, interval);
                let sock = this._setupWebSocket(eventHandler, response.listenKey);
                sock.on('close', () => clearInterval(alive_keeper));
                return sock;
            });
    }
